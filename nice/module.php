...
function widget_tag($title){
	...
	<?php foreach($tag_cache as $key => $value): ?>
		<li style="background:rgb(<?php echo rand(125,190);?>,<?php echo rand(0,125);?>,<?php echo rand(0,125);?>)"><span style="font-size:<?php echo $value['fontsize']; ?>pt; line-height:30px;">
		<a href="<?php echo Url::tag($value['tagurl']); ?>" title="<?php echo $value['usenum']; ?> 篇文章"><?php echo $value['tagname']; ?></a></span></li>
	<?php endforeach; ?>
	</ul>
...
function widget_newcomm($title){
	...
	?>
	  <dl>
        <dt><img src="<?php echo TEMPLATE_URL; ?>style/images/s<?php echo rand(1,8)?>.jpg">
        <dt>
        <dd><?php echo $value['name']; ?>
          <time><?php echo date("Y-m-d",$value['date']); ?></time>
        </dd>
        <dd>评论：</dd>
        <dd><a href="<?php echo $url; ?>"><?php echo $value['content']; ?></a></dd>
      </dl>
...
function widget_hotlog($title){
	...
	$randLogs = $Log_Model->getHotLog($index_hotlognum);?>
		...
		<ul id="hotlog">
		<?php foreach($randLogs as $key => $value): ?>
				<?php if($key==0){ ?>
					<li><span><strong><?php echo $key+1;?></strong></span><a href="<?php echo Url::log($value['gid']); ?>"><?php echo $value['title']; ?></a></li>
					<?php }elseif($key==1){ ?>
					<li><span><strong><?php echo $key+1;?></strong></span><a href="<?php echo Url::log($value['gid']); ?>"><?php echo $value['title']; ?></a></li>
					<?php }else{?>
			<li><span><strong><?php echo $key+1;?></strong></span><a href="<?php echo Url::log($value['gid']); ?>"><?php echo $value['title']; ?></a></li>
...
function widget_search($title){ ?>
	...
	<form class="searchform" method="get" action="<?php echo BLOG_URL; ?>index.php">
		<input type="text" name="keyword" value="Search" onfocus="this.value=''" onblur="this.value='Search'">
		</form>
...
function blog_navi(){
	...
	foreach($navi_cache as $value):
		...
		if($value['url'] == ROLE_ADMIN && (ROLE == ROLE_ADMIN || ROLE == ROLE_WRITER)):
			?>
			<li class="item common"><a href="<?php echo BLOG_URL; ?>zgh528/">管理站点</a></li>
			<li class="item common"><a href="<?php echo BLOG_URL; ?>zgh528/?action=logout">退出</a></li>
			<?php 
			continue;
		endif;
...
function topflg($top, $sortop='n', $sortid=null){
    if(blog_tool_ishome()) {
       echo $top == 'y' ? "<img src=\"".TEMPLATE_URL."/style/images/top.png\" title=\"首页置顶文章\" /> " : '';
    } elseif($sortid){
       echo $sortop == 'y' ? "<img src=\"".TEMPLATE_URL."/style/images/sortop.png\" title=\"分类置顶文章\" /> " : '';
    }
}
?>
<?php
...
function editflg($logid,$author){
	$editflg = ROLE == ROLE_ADMIN || $author == UID ? '<a href="'.BLOG_URL.'zgh528/write_log.php?action=edit&gid='.$logid.'" target="_blank">编辑</a>' : '';
	echo $editflg;
}
?>
<?php
...
function blog_sort($blogid){
    ...
    <li><a href="<?php echo Url::sort($log_cache_sort[$blogid]['id']); ?>"><?php echo $log_cache_sort[$blogid]['name']; ?></a></li>
	<?php endif;?>
<?php }?>
<?php
//blog：文章标签
